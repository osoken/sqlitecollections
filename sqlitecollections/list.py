...
class _ListDatabaseDriver:
    ...
    def get_index_by_serialized_value(self, cur: sqlite3.Cursor, serialized_value: bytes) -> int:
    ...
    def get_serialized_value_by_index(self, cur: sqlite3.Cursor, index: int) -> Union[None, bytes]:
        if index < 0:
    ...
    def tidy_indices(self, cur: sqlite3.Cursor, cur2: sqlite3.Cursor, start: int = 0) -> None:
    ...
    def delete_record_by_index(self, cur: sqlite3.Cursor, index: int, length: Optional[int] = None) -> Union[None, int]:
        _index = index
        if _index < 0:
            _length = length
            if _length is None:
                _length = self.get_max_index_plus_one(cur)
    ...
    def set_serialized_value_by_index(self, cur: sqlite3.Cursor, serialized_value: bytes, index: int) -> bool:
        _index = index
        l = self.get_max_index_plus_one(cur)
        if _index < 0:
            _index = l + index
    ...
    def remap_index(self, cur: sqlite3.Cursor, indices_map: Iterable[int]) -> None:
        l = self.get_max_index_plus_one(cur)
    ...
    def iter_serialized_value(self, cur: sqlite3.Cursor) -> Iterable[bytes]:
    ...
    def count_serialized_value(self, cur: sqlite3.Cursor, serialized_value: bytes) -> int:
        cur.execute(
    ...
    def increment_indices(self, cur: sqlite3.Cursor, start: int) -> None:
        idx = self.get_max_index_plus_one(cur) - 1
        while idx >= start:
...
class List(SqliteCollectionBase[T], MutableSequence[T]):
    ...
    def __delitem__(self, i: Union[int, slice]) -> None:
        ...
        if isinstance(i, int):
            deleted_index = self._database_driver.delete_record_by_index(cur, i)
            if deleted_index is None:
                raise IndexError("list assignment index out of range")
            self._database_driver.tidy_indices(cur, cur2, deleted_index)
            self.connection.commit()
            return
        reindexing_offset = None
        l = self._database_driver.get_max_index_plus_one(cur)
        for idx in _generate_indices_from_slice(l, i):
            self._database_driver.delete_record_by_index(cur, idx, l)
            if reindexing_offset is None or idx < reindexing_offset:
                reindexing_offset = idx
        if reindexing_offset is not None:
            self._database_driver.tidy_indices(cur, cur2, reindexing_offset)
        self.connection.commit()
    ...
    def __getitem__(self, i: Union[int, slice]) -> "Union[T, List[T]]":
        cur = self.connection.cursor()
        if isinstance(i, int):
            serialized_value = self._database_driver.get_serialized_value_by_index(cur, i)
        ...
        for next_idx, idx in enumerate(_generate_indices_from_slice(l, i)):
            buf._database_driver.add_record_by_serialized_value_and_index(
                bufcur, cast(bytes, self._database_driver.get_serialized_value_by_index(cur, idx)), next_idx
            )
    ...
    def __setitem__(self, i: Union[int, slice], v: Union[T, Iterable[T]]) -> None:
        cur = self.connection.cursor()
        if isinstance(i, int):
            if not self._database_driver.set_serialized_value_by_index(cur, self.serialize(cast(T, v)), i):
                raise IndexError("list assignment index out of range")
            self.connection.commit()
            return
        ...
        if i.step is None or i.step == 1:
            ...
            try:
                for idx, d in _strict_zip(_generate_indices_from_slice(l, i), v):
                    self._database_driver.set_serialized_value_by_index(cur, self.serialize(d), idx)
    ...
    def __len__(self) -> int:
        cur = self.connection.cursor()
        return self._database_driver.get_max_index_plus_one(cur)

    def insert(self, i: int, v: T) -> None:
        cur = self.connection.cursor()
        index_ = i
        length = self._database_driver.get_max_index_plus_one(cur)
        if index_ < 0:
            index_ = length + index_
        index_ = max(0, min(length, index_))
        self._database_driver.increment_indices(cur, index_)
        self._database_driver.add_record_by_serialized_value_and_index(cur, self.serialize(v), index_)
        self.connection.commit()

    def __contains__(self, x: object) -> bool:
        cur = self.connection.cursor()
        serialized_value = self.serialize(cast(T, x))
        index = self._database_driver.get_index_by_serialized_value(cur, serialized_value)
        return index != -1

    def append(self, value: T) -> None:
        cur = self.connection.cursor()
        length = self._database_driver.get_max_index_plus_one(cur)
        self._database_driver.add_record_by_serialized_value_and_index(cur, self.serialize(value), length)
    ...
    def extend(self, values: Iterable[T]) -> None:
        cur = self.connection.cursor()
        idx = self._database_driver.get_max_index_plus_one(cur)
        for v in values:
            self._database_driver.add_record_by_serialized_value_and_index(cur, self.serialize(v), idx)
            idx += 1
        self.connection.commit()
    ...
    def __imul__(self, i: int) -> "List[T]":
        ...
        if i == 1:
            return self
        cur = self.connection.cursor()
        original_length = self._database_driver.get_max_index_plus_one(cur)
        for m in range(1, i):
            for j in range(original_length):
                serialized_value = cast(bytes, self._database_driver.get_serialized_value_by_index(cur, j))
                self._database_driver.add_record_by_serialized_value_and_index(
    ...
    def index(self, value: Any, start: int = 0, stop: int = 0) -> int:
        ...
        res = self._database_driver.get_index_by_serialized_value_in_range(cur, serialized_value, start_, stop_)
        if res is None:
    ...
    def count(self, value: Any) -> int:
        cur = self.connection.cursor()
        return self._database_driver.count_serialized_value(cur, self.serialize(cast(T, value)))
    ...
    def pop(self, index: int = -1) -> T:
        ...
        serialized_value = cast(bytes, self._database_driver.get_serialized_value_by_index(cur, index_))
        self._database_driver.delete_record_by_index(cur, index_)
        self._database_driver.tidy_indices(cur, cur2, index_)
        self.connection.commit()
        return self.deserialize(serialized_value)
    ...
    def remove(self, value: T) -> None:
        cur = self.connection.cursor()
        cur2 = self.connection.cursor()
        index = self._database_driver.get_index_by_serialized_value(cur, self.serialize(value))
        if index == -1:
            raise ValueError(f"'{value}' is not in list")
        self._database_driver.delete_record_by_index(cur, index)
        self._database_driver.tidy_indices(cur, cur2, index)
        self.connection.commit()
        return None
